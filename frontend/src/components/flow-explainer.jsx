...
/**
 * Boxed, looping explainer of one CCIP-Read resolution: wallet -> ENS resolver
 * -> gateway -> (Sui + offchain) -> aggregator -> signed response -> wallet.
 *
 * Ported from the demo app's `flow-explainer.tsx`. The two apps ship separately
 * (this one is a static IPFS build), so the component is duplicated rather than
 * shared through a package.
 */
    ...
    >
      <div className={styles.scroll}>
        ...
        >
            ...
            return (
              <g className={styles.chip} key={`${beat}-c-${e.id}`}>
                <rect
                  ...
                  y={e.chip.y - CHIP_H / 2}
                />
                <text x={e.chip.x} y={e.chip.y + 3.5}>
                  {e.chip.text}
                </text>
              </g>
            );
          ...
          {NODES.map((n) => (
            <g className={styles.node} data-on={isOn(n.id)} data-tone={n.tone} key={n.id}>
              ...
              {n.sub ? (
                <text className={styles.nodeSub} x={n.x + n.w / 2} y={n.y + n.h / 2 + 14}>
                  {n.sub}
                </text>
              ) : null}
            </g>
